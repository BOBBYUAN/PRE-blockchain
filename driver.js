...
// Clients
let alice = new Client({name: "Alice", net: fakeNet, secretData: "My Big Secret"});
let bob = new Client({name: "Bob", net: fakeNet});
let charlie = new Client({name: "Charlie", net: fakeNet});
let proxy = new Miner({name: "Proxy", net: fakeNet});
// Miners
...
// Alice sends her private data onlt to bob, that encrypted by Bob's pubkey
// Bob decrypt with his prikey and makes it his own private data
// console.log();
// console.log("Before Bob has secret: " + bob.secretData);
// alice.postEncrytTransaction({receiver: bob});
// console.log();
// console.log("After Bob has secret: " + bob.secretData);
// console.log();


// Alice send the secret data to the proxy
console.log();
console.log("Before Proxy has secret: " + proxy.secretData);
alice.postEncrytTransaction(proxy,[{ amount: 40, address: proxy.address }]);
console.log();
console.log("After Proxy has secret: " + proxy.secretData);
console.log();

// Now Bob wants to buy the context of Alice's data
// This part should be involved with some negotiate price
// and also some validation
// bob.postTransaction([{ amount: 40, address: alice.address }]);

// Alice received the money that from Bob, now she generates the 
// re-encryption and send to Proxy
// alice.postReKeyTransaction(proxy);
