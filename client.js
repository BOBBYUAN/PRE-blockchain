...
let PRE = require('bm-pre');

let JSRE = require('recrypt-js');
...
module.exports = class Client extends EventEmitter {
   ...
   */
  constructor({name, net, startingBlock, keyPair, secretData} = {}) {
    super();
  ...
  encryptRequestData(receiver_pubkey) {
    let ciphertext = utils.encrypt(receiver_pubkey,Buffer.from(this.secretData));
    return ciphertext;
  }

  decryptSenderData(cipher) {
    let plaintext = utils.decrypt(this.keyPair.private,cipher);
    this.saveEncryptedData(plaintext);
  }

  saveEncryptedData(secret) {
    this.secretData = secret;
  }

  generateReEncryptionKey(receiver) {
    let reKey = PRE.rekeyGen(this.keyPair.private, receiver.keyPair.public,{returnHex: false});
    return reKey;
  }

  // send encrypt data to another party
  postEncrytTransaction(receiver) {
    PRE.init({g: "this is g", h: "that is h", returnHex: true}).then(params => {
      console.log(params);

      // console.log(this.keyPair.public);
      // console.log(this.keyPair.private);

      let cipher = PRE.enc(this.secretData, this.keyPair.public, params, {returnHex: true});
      console.log(cipher);
      receiver.saveEncryptedData(cipher);
      return this.postGenericTransaction({
          cipher: cipher
      });
    }).catch(err => {
    ...
    // var pk_A = JSRE.Proxy.to_hex(this.keyPair.public.to_bytes());
    // let cipher = JSRE.encryptData(pk_A, this.secretData);
    // console.log(cipher);
  }

  postEncrytTransactionByFee(receiver, outputs, fee=Blockchain.DEFAULT_TX_FEE) {
    // We calculate the total value of gold needed.
    let totalPayments = outputs.reduce((acc, {amount}) => acc + amount, 0) + fee;
    ...
    let receiver_pubkey = receiver.keyPair.public;
    let cipher = this.encryptRequestData(receiver_pubkey);
    console.log("The cipher is: " + cipher);
    receiver.decryptSenderData(cipher);

    // Create and broadcast the cipher into the transaction.
    return this.postGenericTransaction({
      outputs: outputs,
      fee: fee,
      cipher: cipher
  ...
  postReKeyTransaction(proxy) {
    let reKey = this.generateReEncryptionKey(proxy);
    console.log("The Re-Encryption Key is: " + reKey);
    // Create and broadcast cipher into the transaction.
    proxy.reKey = reKey;
    return this.postGenericTransaction({
        cipher: reKey
    });
  ...
  receiveBlock(block) {
    ...
    // Make sure that we have the previous d, unless it is the genesis block.
